...
void HyPar::savePreprocessResults(const std::string &filename) {
    std::ofstream outFile(filename, std::ios::binary);
    if (!outFile) {
        std::cerr << "Failed to open file for saving preprocess results: " << filename << std::endl;
        return;
    }
    ...
    outFile.close();
    std::cout << "Preprocess results saved to " << filename << std::endl;
}

void HyPar::loadPreprocessResults(const std::string &filename) {
    std::ifstream inFile(filename, std::ios::binary);
    if (!inFile) {
        std::cerr << "Failed to open file for loading preprocess results: " << filename << std::endl;
        return;
    }
    ...
    inFile.close();
    std::cout << "Preprocess results loaded from " << filename << std::endl;
}

void HyPar::savePreprocessResultsAsText(const std::string &filename) {
    ...
    outFile.close();
    std::cout << "Preprocess results saved as text to " << filename << std::endl;
}

void HyPar::loadPreprocessResultsAsText(const std::string &filename) {
    ...
    if (line.empty() || line.find("Existing Nodes:") == std::string::npos) {
    ...
    // Load the deleted_nodes set
    std::getline(inFile, line);
    if (line.empty() || line.find("Deleted Nodes:") == std::string::npos) {
    ...
    if (deletedNodesSize == 0) {
        std::getline(inFile, line);
        if (!line.empty()) {
            std::cerr << "Expected an empty line after deletedNodesSize 0, but got: " << line << std::endl;
            return;
        }
    }
    if (deletedNodesSize > 0) {
    ...
    std::getline(inFile, line);
    std::cout << "line: " << line << std::endl;
    if (line.empty() || line.find("Communities:") == std::string::npos) {
    ...
    inFile.close();
    std::cout << "Preprocess results loaded from text file " << filename << std::endl;
}
