...
void ParFunc::bfs_partition(){
    ...
    for (auto it = nodeVec.begin(); it != nodeVec.end();){
        ...
        for (int i = 0; i < K; ++i){
            if (_fpga_add_try(fpgaVec[cur], *it)){
                ...
                while (!q.empty()){
                    ...
                    for (int net : nodes[u].nets){
                        for (size_t j = 0; j < nets[net].size; ++j){
                            ...
                            if (nodes[v].fpga == -1 && _fpga_add_try(fpgaVec[cur], v)){ // @warning: obviously, the connection is just a rough estimation
    ...
    if (!nodeVec.empty()){
        for(auto node : nodeVec){
            ...
            fpgas[nodes[node].fpga].valid=_fpga_add_force(nodes[node].fpga, node);
        }
    }
}
